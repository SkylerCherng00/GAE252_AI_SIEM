...
'''),
    ...
    # Read the file content if file_name is provided
    file_name = file_name.split('.')[0]  # Remove file extension for report naming
    print(f"Comparison report for file: {file_name}")
    file_name = file_name + '_report.md'
    file_path = Path(f"{os.path.abspath(os.path.dirname(__file__))}//validation//reports//{file_name}") 
    print(f"Open comparison report to: {file_path}")
    with open(file_path, 'r', encoding='utf-8') as f:
        report_answer = f.read()
    
    result = preview_chain.invoke({"input": report, "answer": report_answer,"lang": language_code})
    data = {
        'input_report': report,
            'comparison_result': result.content,
            'analyzing_time': analyzing_time,
            'answer_report': file_name,
            'use_model': APP_STATE.current_executor_type,
            "timestamp": datetime.datetime.now().isoformat(),
            }
    _write_to_mongodb(collection_name='DebugComparisonReport', data=data)
    print(f"Comparison report written to MongoDB collection 'DebugComparisonReport' with file name: {file_name}")
...
def _analyze_logs(
    ...
    try:
        ...
        _comparison_report(result_json.get('analysis_report', 'EMPTY analysis report'),
                           ...
                           analyzing_time=(datetime.datetime.now() - start_time).total_seconds(),
                           file_name=file_name)
...
async def analyze_logs_upload(file: UploadFile = File(...), language_code: Optional[str] = 'zh'):
    ...
    try:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            logs = f.read()
        print(f"File '{file.filename}' read successfully. Starting analysis...")
...
if __name__ == "__main__":
    uvicorn.run("debug_agent:app", host="0.0.0.0", port=8000)
