...
def _comparison_report(report: str, language_code: Optional[str] = 'zh') -> str:
...
'''),
        ("human", "Preview the report:\n\n{input}")
    ])
    preview_chain = preview_prompt | APP_STATE.llm
    preview_result = preview_chain.invoke({"input": report, "lang": language_code})
...
def _analyze_logs(logs: str, collection_name: Optional[str] = "SecurityCriteria", top_k: Optional[int] = 5, language_code: Optional[str] = 'zh') -> str:
    ...
    try:
        ...
        result_json['processing_time'] = (datetime.datetime.now() - start_time).total_seconds()
        result_json['comparison_report'] = _comparison_report(result, language_code=language_code)
