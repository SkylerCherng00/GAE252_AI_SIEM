...
class OllamaEmbedding(EmbeddingModel):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            if not embeddings:
                print("Failed to initialize Ollama embeddings")
                return []
                
            return embeddings.embed_query(text)
        except Exception as e:
            print(f"Error generating embeddings from Ollama: {str(e)}")
            return []
    ...
    def embed_documents(self, documents: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            print(f"Error generating document embeddings from Ollama: {str(e)}")
            return []
...
class AzureOpenAIEmbedding(EmbeddingModel):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            if not embeddings:
                print("Failed to initialize Azure OpenAI embeddings")
                return []
                
            return embeddings.embed_query(text)
        except Exception as e:
            print(f"Error generating embeddings from Azure OpenAI: {str(e)}")
            return []
    ...
    def embed_documents(self, documents: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            print(f"Error generating document embeddings from Azure OpenAI: {str(e)}")
            return []
...
class GeminiEmbedding(EmbeddingModel):
    ...
    def embed_query(self, text: str) -> List[float]:
        ...
        try:
            ...
            print(f"Error generating embeddings from Gemini: {str(e)}")
            return []
    ...
    def embed_documents(self, documents: List[str]) -> List[List[float]]:
        ...
        try:
            ...
            print(f"Error generating document embeddings from Gemini: {str(e)}")
            return []
    ...
    def __init__(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ Configuration loaded from API: {CONFIG_EMBED_URL}")
...
if __name__ == "__main__":
    ...
    if embedding_model is None:
        print("No embedding model could be created. Please check your configuration.")
        print("Make sure at least one embedding service is properly configured and available.")
        # Try each provider explicitly
        for provider in ['azure', 'gemini', 'ollama']:
            print(f"Trying to create {provider} embedding model...")
    ...
    if embedding_model is not None:
        ...
        print(f"Embedding model type: {type(model).__name__}")

        # Example usage for a single query
        query_text = "This is a test query for embeddings"
        query_embedding = embedding_model.embed_query(query_text)
        print(f"Query embedding dimensions: {len(query_embedding) if query_embedding else 0}")
        ...
        print(f"Number of document embeddings: {len(doc_embeddings) if doc_embeddings else 0}")
        if doc_embeddings and len(doc_embeddings) > 0:
            print(f"Dimensions of first document embedding: {len(doc_embeddings[0])}")
    else:
        print("Error: No embedding model is available. Please check your configuration and ensure at least one service is running.")
